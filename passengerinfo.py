...
class pembayaran:
    def pembayaran_tiket():
        passenger_registration = PassengerRegistration()
        passenger_registration.getPassengerInfo()
        metode_pembayaran = input("Pembayaran tiket bus menggunakan [cash/e-wallet]: ").lower()

        def generate_kode_booking():
            return ''.join(random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', k=8))
        if metode_pembayaran == "cash":
            ...
            def verify_e_wallet(kode_verifikasi):
                return len(kode_verifikasi) == 6 and kode_verifikasi.isdigit()
            if verify_e_wallet(kode_verifikasi):
                nomor_kursi = verify_e_wallet.randint(1, 30)
                nomor_bus = verify_e_wallet.randint(1, 10)
